...
|分析、统计和压缩|查看当前文件夹中各个格式占比、大小，文件夹的创建日期等信息|筛选结果支持导出为`Xlsx`文件或压缩包|
...
|批量重命名|筛选结果批量重命名，自定义重命名格式（`正规则表达式`）|可独立使用|
|批量格式转换|筛选结果批量格式转换，可以设置转换质量和背景色|可独立使用；**`2.0`新增**|
|文件名拆分|对文件名拆分为文本块，可以自由选择复制或快速上屏|**`2.0.3`新增**
...
* 蓝奏云（随后更新）：https://wwza.lanzouo.com/io5sv32pi6bg
